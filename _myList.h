...
struct notstd_allocator_traits
{
    ...
    //template <typename T>
    using rebind_alloc=typename Alloc::rebind;
...
class MyList
{
    ...
    using NodeAlloc = typename std::allocator_traits<Alloc>::template rebind_traits<Node<T>>;
...
public:
    ...
    {
        Node<T>* newNode = std::allocator_traits<NodeAlloc>::allocate(m_alloc,1);
        ...
        //std::cout<<"push_back"<<m_size<<" znach"<<m_tail->previous->val<<" addr"<<newNode<<std::endl;
    }
    ...
    void pop_back()
    {
        ...
        std::allocator_traits<NodeAlloc>::deallocate(m_alloc,temp,1);
        m_size--;
